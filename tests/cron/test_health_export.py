...
def _lanes(dd, *, errored=(), offmodel=(), invalid=()):
...
def test_metrics_and_alert(tmp_path, monkeypatch):
    ...
    assert "okengine_health_overall 3" in prom            # red (1 errored + grounding 41%)
...
def test_stale_or_missing_monitor_exports_red(tmp_path, monkeypatch):  # invariant-audit #9
    ...
    assert "okengine_health_overall 3" in prom, "dead monitor must export RED, not green"
...
def test_webhook_failure_does_not_advance_state(tmp_path, monkeypatch):  # invariant-audit #20
    ...
    assert state.get("overall", 0) != 3, "dedup state advanced despite webhook failure — RED alert lost"
...
def test_heartbeat_gauge_emitted_even_when_monitor_dead(tmp_path, monkeypatch):  # invariant-audit B6.3
    ...
    assert "okengine_health_overall 3" in prom              # emitted alongside the RED dead-monitor verdict


def test_stale_only_is_yellow_and_critical_stale_is_orange(tmp_path, monkeypatch):
    dd = tmp_path / "wiki" / "dashboards"
    dd.mkdir(parents=True)
    _dash(dd, "fleet-health",
          "- 🟢 ok: 50 · 🟡 stale: 1 · 🟠 critical-stale: 0 "
          "· 🔴 errored: 0 · 🔴 off-model: 0")
    _run(tmp_path, monkeypatch)
    prom = (tmp_path / "metrics" / "okengine.prom").read_text()
    assert "okengine_health_overall 1" in prom
    assert "okengine_fleet_lanes_stale 1" in prom

    _dash(dd, "fleet-health",
          "- 🟢 ok: 50 · 🟡 stale: 0 · 🟠 critical-stale: 1 "
          "· 🔴 errored: 0 · 🔴 off-model: 0")
    _run(tmp_path, monkeypatch)
    prom = (tmp_path / "metrics" / "okengine.prom").read_text()
    assert "okengine_health_overall 2" in prom
    assert "okengine_fleet_lanes_critical_stale 1" in prom
...
def test_invalid_urls_sidecar_shapes_and_all_lane_transition_types(tmp_path, monkeypatch):
    import urllib.request
    dd = tmp_path / "wiki/dashboards"; dd.mkdir(parents=True)
    _dash(dd, "fleet-health",
          "- 🟢 ok: 1 · 🟡 stale: 0 · 🟠 critical-stale: 1 "
          "· 🔴 errored: 0 · 🔴 off-model: 1")
    sidecar = dd / ".fleet-lanes.json"
    sidecar.write_text(json.dumps({"stale": [], "critical-stale": ["critical"],
                                   "errored": [], "off-model": ["off"]}))
    monkeypatch.setenv("OKENGINE_HEARTBEAT_URL", "not-a-url")
    _run(tmp_path, monkeypatch)
    alerts = (dd / "alerts.md").read_text()
    assert "newly OFF-MODEL: off" in alerts
    assert "CRITICAL-STALE: critical" in alerts

def test_sidecar_invalid_list_fallback_offmodel_and_heartbeat_failure(tmp_path, monkeypatch, capsys):
    import urllib.request
    dd = tmp_path / "wiki/dashboards"; dd.mkdir(parents=True)
    _dash(dd, "fleet-health", "- 🟢 ok: 1 · 🔴 errored: 0 · 🔴 off-model: 1")
    (dd / ".fleet-lanes.json").write_text(json.dumps({
        "stale": [], "critical-stale": [], "errored": [], "off-model": [1]}))
    monkeypatch.setenv("OKENGINE_HEARTBEAT_URL", "https://heartbeat.example/ping")
    monkeypatch.setattr(urllib.request, "urlopen", lambda *_a, **_k: (_ for _ in ()).throw(
        OSError("heartbeat down")))
    _run(tmp_path, monkeypatch)
    assert "fell OFF-MODEL" in (dd / "alerts.md").read_text()
    assert "heartbeat ping failed" in capsys.readouterr().err


def test_invalid_schedule_identity_and_count_fallback_both_alert(tmp_path, monkeypatch):
    dd = tmp_path / "wiki/dashboards"
    dd.mkdir(parents=True)
    _dash(dd, "fleet-health",
          "- 🟢 ok: 1 · 🔴 invalid-schedule: 1 · 🔴 errored: 0 · 🔴 off-model: 0")
    _lanes(dd, invalid=("bad-cron",))
    _run(tmp_path, monkeypatch)
    assert "newly INVALID-SCHEDULE: bad-cron" in (dd / "alerts.md").read_text()

    (dd / ".fleet-lanes.json").unlink()
    state = tmp_path / "metrics/.health-state.json"
    saved = json.loads(state.read_text())
    saved["invalid_schedule"] = 0
    saved.pop("invalid_schedule_lanes", None)
    state.write_text(json.dumps(saved), encoding="utf-8")
    _run(tmp_path, monkeypatch)
    assert "has an INVALID-SCHEDULE (now 1)" in (dd / "alerts.md").read_text()


def test_lane_sidecar_rejects_invalid_schedule_shape(tmp_path, monkeypatch):
    dd = tmp_path / "wiki/dashboards"
    dd.mkdir(parents=True)
    _dash(dd, "fleet-health", "- 🟢 ok: 1 · 🔴 errored: 0 · 🔴 off-model: 0")
    _lanes(dd)
    sidecar = dd / ".fleet-lanes.json"
    payload = json.loads(sidecar.read_text())
    payload["invalid-schedule"] = [1]
    sidecar.write_text(json.dumps(payload), encoding="utf-8")

    monkeypatch.setenv("WIKI_PATH", str(tmp_path))
    spec = importlib.util.spec_from_file_location(
        "health_export_invalid_sidecar", REPO / "scripts/cron/health_export.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    assert module._fleet_lane_sets() is None
