...
PLAIN = ("- 🟢 ok: 88 · 🟡 stale: 3 · 🟠 critical-stale: 1 "
         "· 🔴 errored: 1 · 🔴 invalid-schedule: 4 · 🔴 off-model: 2 · 🟡 never-run: 0\n")


def test_parse_plain_counts():
    # regression: the shipped bold-only regex parsed all of these as None -> exported 0
    assert he._parse_fleet(PLAIN) == {
        "stale": 3, "critical-stale": 1, "errored": 1,
        "invalid-schedule": 4, "off-model": 2, "ok": 88}
...
def test_parse_tolerates_bold_too():
    bold = ("🟢 ok: **88** · 🟡 stale: **3** · 🟠 critical-stale: **2** "
            "· 🔴 errored: **1** · 🔴 invalid-schedule: **0** · 🔴 off-model: **0**\n")
    assert he._parse_fleet(bold) == {
        "stale": 3, "critical-stale": 2, "errored": 1,
        "invalid-schedule": 0, "off-model": 0, "ok": 88}
...
def test_producer_still_emits_plain_counts():
    ...
    for frag in ("ok: {counts['ok']}", "stale: {counts['stale']}",
                 "critical-stale: {counts['critical-stale']}",
                 "errored: {counts['errored']}",
                 "invalid-schedule: {counts['invalid-schedule']}",
                 "off-model: {counts['off-model']}"):
...
def test_fleet_health_emits_lane_identity_sidecar(tmp_path, monkeypatch):
    ...
    (logs / "lane-error-20260723-010000.log").write_text(
        "ERROR cron-plus.runner: agent run failed: Script exited\n"
    )
    (logs / "lane-ok-20260723-010000.log").write_text(
        "INFO cron-plus.runner: cron-plus runner completed (job=lane-ok)\n"
    )
