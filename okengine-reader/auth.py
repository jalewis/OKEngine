"""Authorization and exposure policy for the read-only reader surface."""
...
def exposure_refusal(trust: str, bind_host: str, password: str) -> str | None:
    ...
    if bind_host.strip() in ("", "127.0.0.1", "localhost", "::1") or password:
        return None
    return (
        f"okengine-reader REFUSED to start: PRIVATE vault exposed on {bind_host.strip()!r} with no "
        "OKENGINE_READER_PASSWORD. Set a reader password, bind to loopback "
        "(OKENGINE_BIND=127.0.0.1), or declare the pack `trust: public`. (okengine#90 P4a)"
    )
