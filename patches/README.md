...
| 04 | `04-usage-pricing-models.patch` | `agent/usage_pricing.py` | DeepSeek rates. Adds `deepseek-v4-flash` (upstream lacks it, so `insights` cost tracking would read `unknown`) and CORRECTS upstream's `deepseek-v4-pro` entry, whose 2026-05 snapshot was wrong in both directions at once — input over-stated against off-peak, cache-read under-stated ~3x. Both carry **peak** rates: DeepSeek bills peak (01:00-04:00 + 06:00-10:00 UTC) at 2x off-peak, this table has one rate per model, and under-stating cost is the dangerous direction. (Claude 4.x is upstream-native as of v0.18.0.) |
