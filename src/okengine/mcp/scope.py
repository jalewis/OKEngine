...
def path_in_scopes(rel_path: str, scopes) -> bool:
    ...
    rp = raw_path.lstrip("/")
    # Scope matching is an authorization boundary, not a path resolver. Never let traversal,
    # Windows separators, or NUL-bearing input match a permissive prefix/full-vault glob; the
    # eventual filesystem guard is defense in depth and may live in a different service.
    if "\\" in raw_path or "\0" in raw_path or any(part in {".", ".."} for part in rp.split("/")):
        return False
    if rp.endswith(".md"):
        rp = rp[:-3]
